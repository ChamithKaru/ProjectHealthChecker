...
class SOM(object):
    ...
    def view_U_matrix(self, dlen, distance2=2, row_normalized='Yes', show_data='Yes', contooor='Yes', blob='No',
                      save='No',
                      save_dir=''):
        import scipy
        ...
        if show_data == 'Yes':
            labels = ['{0}'.format(i) for i in range(dlen)]
            plt.subplots_adjust(bottom=0.1)
            plt.scatter(coord[:, 1], coord[:, 0], s=2, alpha=1., c='Gray', marker='o', linewidths=3,
                        edgecolor='Gray', cmap=plt.get_cmap('Spectral'))
